...
$(function() {
    ...
    var adtimer;
    var _wrap = $("#pic-ad ol"); //
    var len = $("#pic-ad ol li").length;
    //len=1;
    if (len > 1) {
        $("#pic-ad").hover(function() {
            clearInterval(adtimer);
        },
        function() {
            adtimer = setInterval(function() {
        ...
        function showImg(index) {
            var Height = $("#pic-ad").height();
            $("#pic-ad ol").stop().animate({
                marginTop: -_h + 'px'
            },
            1000);
        }

        $("#pic-ad").mouseover(function() {
            $("#pic-ad .mouse_direction").css("display", "block");
        });
        $("#pic-ad").mouseout(function() {
            $("#pic-ad .mouse_direction").css("display", "none");
        });
    }

    $("#pic-ad").find(".mouse_top").click(function() {
        var _field = _wrap.find('li:first'); //此变量不可放置于函数起始处,li:first取值是变化的
        var last = _wrap.find('li:last'); //此变量不可放置于函数起始处,li:last取值是变化的
        //last.prependTo(_wrap);
        var _h = last.height();
        $("#pic-ad ol").css('marginTop', -_h + "px");
        last.prependTo(_wrap);
        $("#pic-ad ol").animate({
            marginTop: 0
        },
        500,
        function() { //通过取负margin值,隐藏第一行
            //$("#pic-ad ol").css('marginTop',0).prependTo(_wrap);//隐藏后,将该行的margin值置零,并插入到最后,实现无缝滚动
        })
    });
    $("#pic-ad").find(".mouse_bottom").click(function() {
        var _field = _wrap.find('li:first'); //此变量不可放置于函数起始处,li:first取值是变化的
        var _h = _field.height();
        _field.animate({
            marginTop: -_h + 'px'
